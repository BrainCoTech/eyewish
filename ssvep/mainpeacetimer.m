...
% -------------------------------------------------------------------------
% 各目标的刺激频率
freq = [18.5,18.8,19.1,19.4,19.7,20,20.3,20.6,20.9,21.2];
% 刺激呈现的时间
displayTime = 2;
restDelay = 0.4;
trialnum = 24;
...
try
  ...
  %  Screen('FillRect',w, black);%黑色底
    Screen('FillRect',w, white*0.28);%黑色底
    
    
    % 显示的图片 读取矩阵 制作纹理
    stim_represent = imread('redsquare.png');
    texturetarget = Screen('MakeTexture', w, stim_represent);%用来标记识别目标
    stim_represent = imread('facegray.png');
    textureGUI = Screen('MakeTexture', w, stim_represent);%第一帧
    stim_represent = imread('light3.png');
    %stim_represent = imread('lightsquare.png');
    textureGUI2 = Screen('MakeTexture', w, stim_represent);%源图帧
    
     rectp1 = [200 200];%刺激块大小
    % ---------------------------------------------------------------------
 
    % 图片1在屏幕上的位置
    p1horiz = params.horiz/2 - rectp1(1)/2 *7;
    p1verti = params.vertical/2 - rectp1(2)/2 - 200+10;
    rect1 = [p1horiz p1verti p1horiz+rectp1(1) p1verti+rectp1(2)];
    %
    p2horiz = params.horiz/2 - rectp1(1)/2 *4;
    p2verti = params.vertical/2 - rectp1(2)/2 - 200+10;
    rect2 = [p2horiz p2verti p2horiz+rectp1(1) p2verti+rectp1(2)];
    %
    p3horiz = params.horiz/2 - rectp1(1)/2;
    p3verti = params.vertical/2 - rectp1(2)/2 - 200+10;
    rect3 = [p3horiz p3verti p3horiz+rectp1(1) p3verti+rectp1(2)];
    %
    p4horiz = params.horiz/2 + rectp1(1)/2 *2;
    p4verti = params.vertical/2 - rectp1(2)/2 - 200+10;
    rect4 = [p4horiz p4verti p4horiz+rectp1(1) p4verti+rectp1(2)];
    %
    p5horiz = params.horiz/2 + rectp1(1)/2 *5;
    p5verti = params.vertical/2 - rectp1(2)/2 - 200+10;
    rect5 = [p5horiz p5verti p5horiz+rectp1(1) p5verti+rectp1(2)];
    
    % 图片6在屏幕上的位置
    p6horiz = params.horiz/2 - rectp1(1)/2 *7;
    p6verti = params.vertical/2 - rectp1(2)/2 - 200+10+300+50+25;
    rect6 = [p6horiz p6verti p6horiz+rectp1(1) p6verti+rectp1(2)];
    %
    p7horiz = params.horiz/2 - rectp1(1)/2 *4;
    p7verti = params.vertical/2 - rectp1(2)/2 - 200+10+300+50+25;
    rect7 = [p7horiz p7verti p7horiz+rectp1(1) p7verti+rectp1(2)];
    %
    p8horiz = params.horiz/2 - rectp1(1)/2;
    p8verti = params.vertical/2 - rectp1(2)/2 - 200+10+300+50+25;
    rect8 = [p8horiz p8verti p8horiz+rectp1(1) p8verti+rectp1(2)];
    %
    p9horiz = params.horiz/2 + rectp1(1)/2 *2;
    p9verti = params.vertical/2 - rectp1(2)/2 - 200+10+300+50+25;
    rect9 = [p9horiz p9verti p9horiz+rectp1(1) p9verti+rectp1(2)];
    %
    p10horiz = params.horiz/2 + rectp1(1)/2 *5;
    p10verti = params.vertical/2 - rectp1(2)/2 - 200+10+300+50+25;
    rect10 = [p10horiz p10verti p10horiz+rectp1(1) p10verti+rectp1(2)];
    
    
    cond_horiz = {'p1horiz','p2horiz','p3horiz','p4horiz','p5horiz','p6horiz','p7horiz','p8horiz','p9horiz','p10horiz'};
    cond_verti = {'p1verti','p2verti','p3verti','p4verti','p5verti','p6verti','p7verti','p8verti','p9verti','p10verti'};
    recti = {'rect1','rect2','rect3','rect4','rect5','rect6','rect7','rect8','rect9','rect10'};
    % ---------------------------------------------------------------------
