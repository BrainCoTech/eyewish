...
%%
% TCPIP连接设置
interfaceObject=tcpip('192.168.28.170',10001,'NetworkRole','server');%与第一个请求连接的客户机建立连接，端口号为10008，类型为服务器。
interfaceObject.InputBuffersize=33*n;%缓存
interfaceObject.RemoteHost='192.168.28.60';%客户端ip
% 设置一次读取的字节数
bytesToRead = 33*n;
...
sp1=subplot(9,1,1);
plotHandle1 = plot(0,'-','LineWidth',1,'color',[1,0,0]);
sp1.XAxis.Visible = 'off';
grid minor

sp2=subplot(9,1,2);
plotHandle2 = plot(0,'-','LineWidth',1,'color',[1,0,0]);
sp2.XAxis.Visible = 'off';
grid minor

%  定义当输入缓冲区中达到所需字节数时要执行的回调函数  注意：回调函数必须在开启服务之前
interfaceObject.BytesAvailableFcn = {@read,plotHandle1,plotHandle2,bytesToRead};%可读字节数回调函数，当可读取字节数超过一定范围或者接收特定的结束符时候才调用
...
function read(interfaceObject,~,plotHandle1,plotHandle2,bytesToRead)
global x;
global ch1;
global ch2;
global ch3;
